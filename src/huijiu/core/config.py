...
ROBOT_PORT = "COM4"
ROBOT_BAUD = 115200
ROBOT_URDF = PROJECT_ROOT / "hardware" / "urdf" / "mycobot_280m5_with_gripper_up.urdf"

#相机工作频率 FPS 全称是 Frames Per Second，也就是“每秒多少帧”。
D455_FPS = 30
#相机内参数据（使用棋盘格进行数据采集生成）
D455_INSTRINSICS = PROJECT_ROOT / "hardware" / "calib" / "d455_intrinsic.json"

HAND_EYE_PATH = PROJECT_ROOT / "hardware" / "calib" / "handeye_extrinsics.json"
#手眼标定，采用相机+机械臂+ArUco码标定采集生成
CALIB_JSON = PROJECT_ROOT / "hardware" / "calib" / "handeye_samples.json"
#手眼标定
OUT_JSON = CALIB_JSON.with_name("handeye_result.json")
...
def load_samples(path: Path):
    ...
    for i, s in enumerate(raw):
        ...
        robot_pose = s["robot"]              # 例如 [x, y, z, rx, ry, rz]
        marker_rvec = np.array(s["marker_rvec"], dtype=np.float64)
        marker_tvec = np.array(s["marker_tvec"], dtype=np.float64).reshape(3, 1)
